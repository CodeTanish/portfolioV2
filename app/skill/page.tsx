...
const DetailCard: React.FC<DetailCardProps> = ({ title, onBack, skills }) => (
  ...
  >
    <h2 className="text-3xl font-bold mb-10 tracking-wide text-center text-black dark:text-white">
      {title}
    </h2>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 w-full">
      {skills.map((skill, index) => (
        ...
        >
          <div className="w-16 h-16 mb-4 relative overflow-hidden">
            <Image
              src={skill.image}
              alt={skill.title}
              fill
          ...
          <p className="font-semibold text-lg mb-1 text-black dark:text-white">
            {skill.title}
          </p>
        ...
        {selectedCard === null ? (
          ...
          >
            {Object.keys(skillsData).map((key) => (
              <Card
                key={key}
                title={key as SkillCategory}
                onClick={() => handleCardClick(key as SkillCategory)}
              />
            ))}
          </motion.div>
        ) : (
